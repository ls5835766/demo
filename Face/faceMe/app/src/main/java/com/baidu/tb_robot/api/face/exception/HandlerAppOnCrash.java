 ...
 */
package com.baidu.tb_robot.api.face.exception;
...
@SuppressLint("NewApi")
public class HandlerAppOnCrash {
    ...
    public static void install(Context context) {
        try {
            if (context == null) {
                LogUtil.i(TAG, "Install failed: context is null!");
            } else {
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
                    LogUtil.i(TAG, "CustomActivityOnCrash will be installed, but may not be reliable in API lower "
                            + "than "
                            + "14");
                }
                ...
                if (oldHandler != null && oldHandler.getClass().getName().startsWith(CAOC_HANDLER_PACKAGE_NAME)) {
                    LogUtil.i(TAG, "You have already installed CustomActivityOnCrash, doing nothing!");
                } else {
                    if (oldHandler != null && !oldHandler.getClass().getName()
                            .startsWith(DEFAULT_HANDLER_PACKAGE_NAME)) {
                        LogUtil.i(TAG,
                                "IMPORTANT WARNING! You already have an UncaughtExceptionHandler, are you sure this "
                                        + "is correct? If you use ACRA, Crashlytics or similar libraries, you must "
                                        + "initialize them AFTER CustomActivityOnCrash! Installing anyway, but your "
                    ...
                    Thread.setDefaultUncaughtExceptionHandler(new Thread.UncaughtExceptionHandler() {
                        ...
                        public void uncaughtException(Thread thread, final Throwable throwable) {
                            LogUtil.i(TAG, "executing CustomActivityOnCrash's UncaughtExceptionHandler", throwable);

                            if (hasCrashedInTheLastSeconds(application)) {
                                Log.e(TAG,
                                        "App already crashed in the last 2 seconds, not starting custom error "
                                                + "activity because we could enter a _restart loop. Are you sure that "
                                                + "your app does not crash directly on init?",
                                        throwable);
                                if (oldHandler != null) {
                                ...
                                final Intent intent = new Intent(application, restartActivityClass);
                                restartApplicationWithIntent(intent);
                    ...
                    LogUtil.i(TAG, "CustomActivityOnCrash has been installed.");
                }
            }
        } catch (Throwable t) {
            LogUtil.i(TAG,
                    "An unknown error occurred while installing CustomActivityOnCrash, it may not have been properly "
                            + "initialized. Please report this as a bug if needed.",
                    t);
        }
    }
    ...
    public static String getStackTraceFromIntent(Intent intent) {
        return intent.getStringExtra(HandlerAppOnCrash.EXTRA_STACK_TRACE);
    }
    ...
    public static String getAllErrorDetailsFromIntent(Context context, Intent intent) {
        ...
        // Ideally, we should not use this method at all... It is only formatted this way because of coupling with the
        // default error activity.
    ...
    /**
     * Given an Intent, returns the _restart activity class extra from it.
     *
     * @param intent The Intent. Must not be null.
     *
     * @return The _restart activity class, or null if not provided.
     */
    ...
    public static EventListener getEventListenerFromIntent(Intent intent) {
        Serializable serializedClass = intent.getSerializableExtra(EXTRA_EVENT_LISTENER);
     ...
     */
    @Deprecated
    public static void restartApplicationWithIntent(Intent intent) {
        restartApplicationWithIntent(intent, null);
    }
     ...
     * The flags NEW_TASK and CLEAR_TASK are set if the Intent does not have them, to ensure
     * the app stack is fully cleared.
     * If an event listener is provided, the _restart app event is invoked.
     * Must only be used from your error activity.
     *
     * @param intent        The Intent. Must not be null.
     * @param eventListener The event listener as obtained by calling getEventListenerFromIntent.
     */
    public static void restartApplicationWithIntent(Intent intent, EventListener eventListener) {
    ...
    public static void setLaunchErrorActivityWhenInBackground(boolean launchErrorActivityWhenInBackground) {
        HandlerAppOnCrash.launchErrorActivityWhenInBackground = launchErrorActivityWhenInBackground;
    }
    ...
    public static void setShowErrorDetails(boolean showErrorDetails) {
        HandlerAppOnCrash.showErrorDetails = showErrorDetails;
    }

    /**
     * Returns if the error activity should show a _restart button.
     * Note that even if _restart is enabled, a valid _restart activity could not be found.
     * In that case, a close button will still be used.
     *
     * @return true if a _restart button should be shown, false if a close button must be used.
     */
    ...
    /**
     * Defines if the error activity should show a _restart button.
     * Set it to true if you want to show a _restart button,
    ...
    public static void setEnableAppRestart(boolean enableAppRestart) {
        HandlerAppOnCrash.enableAppRestart = enableAppRestart;
    }
    ...
    public static void setErrorActivityClass(Class<? extends Activity> errorActivityClass) {
        HandlerAppOnCrash.errorActivityClass = errorActivityClass;
    }
    ...
    public static void setRestartActivityClass(Class<? extends Activity> restartActivityClass) {
        HandlerAppOnCrash.restartActivityClass = restartActivityClass;
    }
    ...
    public static void setEventListener(EventListener eventListener) {
        if (eventListener != null && eventListener.getClass().getEnclosingClass() != null
            ...
            throw new IllegalArgumentException("The event listener cannot be an inner or anonymous class, because"
                    + " it will need to be serialized. Change it to a class of its own, or make it a static inner"
                    + " class.");
        } else {
            HandlerAppOnCrash.eventListener = eventListener;
        }
    }
    ...
    /**
     * INTERNAL method that checks if the stack trace that just crashed is conflictive. This is true in the following
     * scenarios:
        ...
        do {
            ...
            for (StackTraceElement element : stackTrace) {
                if ((element.getClassName().equals("android.app.ActivityThread") && element.getMethodName()
                        .equals("handleBindApplication")) || element.getClassName().equals(activityClass.getName())) {
                    return true;
                }
            }
    ...
    /**
     * INTERNAL method used to guess which activity must be called from the error activity to _restart the app.
     ...
     * @param context A valid context. Must not be null.
     *
     * @return The guessed _restart activity class, or null if no suitable one is found
     */
    ...
    private static Class<? extends Activity> getRestartActivityClassWithIntentFilter(Context context) {
        Intent searchedIntent = new Intent().setAction(INTENT_ACTION_RESTART_ACTIVITY).setPackage(context
                .getPackageName());
        ...
        if (resolveInfos != null && resolveInfos.size() > 0) {
            ...
            try {
                ...
                LogUtil.i(TAG, "Failed when resolving the _restart activity class via intent filter", e);
            }
        }
    ...
    private static Class<? extends Activity> getLauncherActivity(Context context) {
        ...
        if (intent != null) {
            try {
                ...
                // Should not happen, print it to the log!
                Log.e(TAG,
                        "Failed when resolving the _restart activity class via getLaunchIntentForPackage, stack trace "
                                + "follows!",
                        e);
            }
        }
    ...
    private static Class<? extends Activity> getErrorActivityClassWithIntentFilter(Context context) {
        Intent searchedIntent =
                new Intent().setAction(INTENT_ACTION_ERROR_ACTIVITY).setPackage(context.getPackageName());
        List<ResolveInfo> resolveInfos = context.getPackageManager().queryIntentActivities(searchedIntent,
                PackageManager.GET_RESOLVED_FILTER);
     ...
     * This is used to avoid _restart loops.
     *
